...
(function initInstagramExtractor() {
	...
	async function extractPostData(article) {
		...
		const claimedDate = extractClaimedDate(description, postDate);
		const claimedLocation = extractClaimedLocation(`${description} ${altTexts.join(" ")}`);
	...
	function renderSendingState(article, extractedPayload) {
		const panel = getOrCreateFloatingPanel(article);
		if (!panel) return;
		const extractedText = JSON.stringify(extractedPayload || {}, null, 2);
		panel.innerHTML = `
			<div class="ig-post-viewer-panel-title">Extracted Fields</div>
			<div class="ig-post-viewer-panel-body">${escapeHtml(extractedText)}\n\nSending to POST API...</div>
		`;
	}

	function renderResultInPanel(article, extractedPayload, result) {
		const panel = getOrCreateFloatingPanel(article);
		if (!panel) return;
		const extractedText = JSON.stringify(extractedPayload || {}, null, 2);
		...
		panel.innerHTML = `
			<div class="ig-post-viewer-panel-title">Extracted Fields + API Response</div>
			<div class="ig-post-viewer-panel-body">Extracted Fields:\n${escapeHtml(extractedText)}\n\nAPI Response:\nStatus: ${result?.status ?? "N/A"}\nSuccess: ${result?.ok ? "Yes" : "No"}\n\n${escapeHtml(bodyText)}</div>
		`;
	}
	...
	function addButtonToArticle(article) {
		...
		button.addEventListener("click", async (event) => {
			...
			const extractedPayload = buildWebhookPayload(data);
			renderSendingState(article, extractedPayload);
			const response = await sendPostPayloadToWebhook(data);
			renderResultInPanel(article, extractedPayload, response);
		});
